...
func main() {
    seed := rand.NewSource(time.Now().UnixNano()) //seed
    randomNumber := rand.New(seed) //random number
    var computerchoice = randomNumber.Intn(2) // make the computer's choice random
